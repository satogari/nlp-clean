...
import {
  ...
  CampaignCatalogInvalidName,
  PartnerMissingError,
  PartnerNotFoundError,
} from '../domain/catalog-campaign.error';
...
import {
  CreateCampaignCatalogUseCaseInteractor,
  CreateCampaignCatalogUseCaseRequest,
} from './create-campaign-catalog.usecase.interactor';
...
describe('CreateCampaignCatalog', () => {
  ...
  beforeEach(() => {
    ...
    inlineMemoryCatalogCampaignRepository = {
      save: jest.fn(),
      generateId: jest.fn(),
    };
    usecase = new CreateCampaignCatalogUseCaseInteractor(
      inlineMemoryPartnerDataProvider,
      inlineMemoryCatalogCampaignRepository,
    );
  });
  ...
  describe('Happy Case', () => {
    it('should return campaign successfully', async () => {
      ...
      const expectedCatalogCampaignId = new CatalogCampaignId(
        '__VALID_CATALOG_CAMPAIGN_ID__',
      );
      ...
      inlineMemoryCatalogCampaignRepository.generateId.mockResolvedValue(
        expectedCatalogCampaignId,
      );
      inlineMemoryCatalogCampaignRepository.save.mockResolvedValueOnce(
        expectedCatalogCampaign,
      );

      const result = await usecase.execute(request);
      expect(result.equal(expectedCatalogCampaign));
    });
  });
