...
export class CreateCampaignCatalogUseCaseInteractor
  implements CreateCampaignCatalogUseCaseInputPort
{
  constructor(
  ...
  async execute(
    ...
    const partnerIds = await this.partnerDataProvider.findByIds(
      request.partnerIds.map((id) => new PartnerId(id)),
    );
